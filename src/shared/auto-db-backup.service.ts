...
export class AutoDbBackupService {
  ...
  handleCron(): void {
    this.loggerService.log('AUTO POSTGRES DB BACKUP CRON JOB');
    ...
    if (!fs.existsSync(this.backupConfig.pgPassFile))
      return this.loggerService.warn('NO PGPASSFILE FILE');
    ...
    if (this.backupConfig.autoBackup == true) {
      let beforeDate: Date,
        ...
        newBackupDir = `${currentDate.getFullYear()}-${currentDate.getMonth() +
      ...
      if (this.backupConfig.removeOldBackup == true) {
        ...
        beforeDate.setDate(
          beforeDate.getDate() - this.backupConfig.keepLastDaysBackup
        );
      ...
      exec(cmd, error => {
        if (error)
          this.loggerService.error('RUNNING DUMP CMD', JSON.stringify(error));
        else this.loggerService.log('POSTGRES');
        if (this.empty(error))
          if (
            this.backupConfig.removeOldBackup == true &&
            fs.existsSync(oldBackupPath)
          )
            // Check for remove old backup after keeping # of days given in configuration
